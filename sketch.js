...
function setup() {
  ...
  //criando o trex
  coiso = createSprite(width, height - 30, width, 12)
  coiso.velocityX = - 8
  coiso.addImage(groundImage)
  trex = createSprite(50, -1000, 20, 50);
  ...
  groundeInvisibl = createSprite(width / 2, height - 22, width, 12)
  groundeInvisibl.visible = false

  rrestrarte = createSprite(width / 2, height / 2 + 50, 50, 50)
  rrestrarte.addImage(rimg)
  rrestrarte.scale = 0.51

  juegoFinale = createSprite(width / 2, height / 2, 50, 50)
...
function draw() {
  ...
  background(235);
  textSize(25)
  text("real Moneh: " + score, width - 450, 50)
  ...
  if (gameState === PLAY) {
    ...
    if (mousePressedOver(rrestrarte) || touches.length > 0) {
      rencar();
      touches = [];
    }
  }
...
function doud() {
  if (frameCount % 60 === 0) {
    cloud = createSprite(width, 150, 20, 20);
    cloud.velocityX = -5
    cloud.y = Math.round(random(height - 100, height - 700));
...
function mexicanos() {
  if (frameCount % 60 === 0) {
    clebito = createSprite(width, height - 45, 20, 40);
